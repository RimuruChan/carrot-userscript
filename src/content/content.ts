import { maybeUpdateContestList, maybeUpdateRatings, predictDeltas } from "./background";
...
function makeGreySpan(text: string, title: string) {
...
function makePerformanceSpan(performance: { value: string, colorClass: string }) {
  const span = document.createElement('span');
...
function makeRankSpan(rank: Rank) {
  const span = document.createElement('span');
  if (rank.colorClass) {
...
function makeArrowSpan(arrow: string) {
  const span = document.createElement('span');
  span.classList.add('small');
  span.style.verticalAlign = 'middle';
...
function makeDeltaSpan(delta: number) {
  const span = document.createElement('span');
  span.style.fontWeight = 'bold';
  span.style.verticalAlign = 'middle';
  if (delta > 0) {
...
function makeFinalRankUpSpan(rank: Rank, newRank: Rank, arrow: string) {
  const span = document.createElement('span');
  span.style.fontWeight = 'bold';
...
function makePredictedRankUpSpan(rank: Rank, deltaReqForRankUp: number, nextRank: Rank | null) {
  const span = document.createElement('span');
  span.style.fontWeight = 'bold';
...
function makeDeltaHeaderCell(deltaColTitle: string) {
  const cell = document.createElement('th');
  cell.classList.add('top');
...
function makeRankUpHeaderCell(rankUpColWidth: string, rankUpColTitle: string) {
  const cell = document.createElement('th');
...
function populateCells(
  row: any,
  type: string,
  rankUpTint: (string | undefined)[],
  perfCell: HTMLElement,
  deltaCell: HTMLElement,
  rankUpCell: HTMLElement
) {
  if (row === undefined) {
...
function updateStandings(resp: any) {
...
function updateColumnVisibility(prefs: any) {
  for (const col of ALL_COLUMNS) {
    const showCol = prefs[col.setting];
    const func =
      showCol ?
        (cell: any) => cell.classList.remove(DISPLAY_NONE_CLS) :
        (cell: any) => cell.classList.add(DISPLAY_NONE_CLS);
...
function showTimer(fetchTime: number) {
  const predictTextSpan = document.getElementById(PREDICT_TEXT_ID)!;
  function update() {
...
async function predict(contestId: number) {
