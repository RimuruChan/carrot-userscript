...
export default class Ratings {
  ...
  async maybeRefreshCache(contestStartMs: number) {
    const inner = async () => {
  ...
  async fetchCurrentRatings(contestStartMs: number): Promise<Map<string, number>> {
  ...
  async cacheRatings() {
    ...
    const ratings = Object.fromEntries(users.map((u: any) => [u.handle, u.rating]));
